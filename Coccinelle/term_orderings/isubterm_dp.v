...
Variable comp : term * term -> nat -> Prop.
Variable comp_def : forall u v, cdp v u -> exists n, comp (u,v) n.
Variable comp_fonc : forall uv n n', comp uv n -> comp uv n' -> n = n'.
Variable comp_ok : forall uv uv', are_connected cdp R uv uv' -> forall n n', comp uv n -> comp uv' n' -> n <= n'.
Definition strong_connect_approx n uv uv' := comp uv n /\ comp uv' n.

Lemma trans_comp_ok : 
  forall uv uv', trans_clos (are_connected cdp R) uv uv' -> forall n n', comp uv n -> comp uv' n' -> n <= n'.
Proof.
intros x y H; induction H as [x y H1 | x y z H1 Hn].
...
Lemma between_dp0_aux2 :
  (forall n u v, strong_connect_approx n (u0,v0) (u,v) ->
...
right; assumption.
subst uv0 uv4.
destruct (comp_def _ _ Hdp4'') as [n4 K4].
apply Hdp with n4; split; [ | assumption].
...
Lemma dp_subterm_criterion_gen :
      forall R : relation term,
       ...
       (forall u0 v0, In (u0,v0) l0 -> cdp v0 u0) ->
       ...
       (forall u0 v0, In (u0,v0) l0 -> trans_clos (union term (one_step R) direct_subterm) (projection P v0)
         (projection P u0)) ->
       forall comp : term * term -> nat -> Prop,
      (forall u v : term, cdp v u -> exists n : nat, comp (u, v) n) ->
     (forall uv n n', comp uv n -> comp uv n' -> n = n') ->
       (forall uv uv' : term * term,
        are_connected cdp R uv uv' ->
        forall n n' : nat, comp uv n -> comp uv' n' -> n <= n') ->
...
intros cdp cdp' cdp_in_dpR P split_cdp l0_diff l0_in_dpR P_ok l0_decr
comp comp_def comp_fonc comp_ok n0 l0_in_comp_n0 Wl0 cdp_decr W'.
...
intros u v [H | H]; apply comp_def.
rewrite split_cdp; left; assumption.
rewrite split_cdp; do 2 right; assumption.
assumption.
